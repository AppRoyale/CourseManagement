...
-- --------------------------------------------------
-- Date Created: 05/11/2013 22:06:46
-- Generated from EDMX file: C:\Users\chris_000\Documents\FH-Flensburg\6. Semester\Software-Projekt\Diamondback\CourseManagement\Client\DB\Model\DiamondbackModel.edmx
-- --------------------------------------------------
...
GO
IF OBJECT_ID(N'[dbo].[FK_RoomCourse]', 'F') IS NOT NULL
    ALTER TABLE [dbo].[Courses] DROP CONSTRAINT [FK_RoomCourse];
GO
...
-- Creating foreign key on [Room_RoomNr] in table 'Courses'
ALTER TABLE [dbo].[Courses]
ADD CONSTRAINT [FK_RoomCourse]
    FOREIGN KEY ([Room_RoomNr])
    REFERENCES [dbo].[Rooms]
        ([RoomNr])
    ON DELETE NO ACTION ON UPDATE NO ACTION;

-- Creating non-clustered index for FOREIGN KEY 'FK_RoomCourse'
CREATE INDEX [IX_FK_RoomCourse]
ON [dbo].[Courses]
    ([Room_RoomNr]);
GO
...
-- Creating foreign key on [Courses_CourseNr] in table 'Appointments'
ALTER TABLE [dbo].[Appointments]
...
-- Creating foreign key on [Rooms_RoomNr] in table 'Appointments'
ALTER TABLE [dbo].[Appointments]
...
-- Creating foreign key on [Tutor_Id] in table 'Appointments'
ALTER TABLE [dbo].[Appointments]
ADD CONSTRAINT [FK_TutorAppointment]
    FOREIGN KEY ([Tutor_Id])
    REFERENCES [dbo].[Persons_Tutor]
        ([Id])
    ON DELETE NO ACTION ON UPDATE NO ACTION;

-- Creating non-clustered index for FOREIGN KEY 'FK_TutorAppointment'
CREATE INDEX [IX_FK_TutorAppointment]
ON [dbo].[Appointments]
    ([Tutor_Id]);
GO
